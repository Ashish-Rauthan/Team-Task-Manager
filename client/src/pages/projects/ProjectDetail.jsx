        ...
        <div style={{ maxHeight: 300, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {fetching ? (
            ...
            >
              ...
              <span style={{ fontSize: '0.75rem', color: 'var(--accent)', fontWeight: 600 }}>+ Add</span>
            </div>
          ))}
        </div>
      </div>
...
function DeleteProjectModal({ project, onClose, onDeleted }) {
  ...
  const isConfirmed = confirm === project.name;
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{
          ...
          <span className="material-symbols-outlined" style={{ color: 'var(--danger)', flexShrink: 0, marginTop: 1 }}>warning</span>
          <div>
            <div style={{ fontWeight: 700, color: 'var(--danger)', fontSize: '0.875rem', marginBottom: 4 }}>
              This action is irreversible
            </div>
            <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              Deleting <strong>{project.name}</strong> will permanently remove all tasks,
              comments, and activity logs associated with this project.
            </div>
          </div>
        </div>
        ...
        <div className="form-group" style={{ marginBottom: 20 }}>
          <label className="form-label">
            Type <strong style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>{project.name}</strong> to confirm
          </label>
          <input
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              background: isConfirmed ? 'var(--danger)' : 'var(--danger-dim)',
              color: isConfirmed ? '#fff' : 'var(--danger)',
              opacity: !isConfirmed ? 0.6 : 1,
            }}
          >
            {loading
              ? <span className="spinner" style={{ borderTopColor: '#fff' }} />
              : <><span className="material-symbols-outlined icon-sm">delete_forever</span> Delete Project</>
            }
          </button>
        </div>
      </div>
...
export default function ProjectDetail() {
  ...
  // isProjectManager: global admin OR has role='manager' in this project
  const isProjectManager = isAdmin ||
    (project?.project_members?.some(m => m.user?.id === user?.id && m.role === 'manager') ?? false);
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ padding: '24px 28px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
              ...
              {/* Show "Project Manager" badge for non-admin managers */}
            ...
            {project.description && (
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9375rem', maxWidth: 560, lineHeight: 1.6 }}>
                {project.description}
              </p>
            )}
          </div>
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <div className="card">
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, marginBottom: 20, fontSize: '0.9375rem' }}>
              Project Details
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[
                ...
                <div key={label} style={{
                  ...
                  fontSize: '0.875rem', paddingBottom: 14, borderBottom: '1px solid var(--border)',
                }}>
                  <span style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {label}
                  </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(project.project_members || []).slice(0, 6).map(m => (
                <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <span style={{ flex: 1, fontSize: '0.875rem', fontWeight: 500 }}>{m.user?.full_name}</span>
                  <span style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9375rem' }}>
              ...
              <span style={{
                ...
                background: 'var(--bg-container-high)', color: 'var(--text-muted)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {(project.project_members || []).map(m => (
              <div key={m.id} style={{
                ...
                padding: '12px 10px', borderRadius: 'var(--radius-md)', transition: 'background 0.15s',
              }}
              ...
              >
                <Avatar name={m.user?.full_name} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: '0.875rem' }}>{m.user?.full_name}</div>
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>{m.user?.email}</div>
                </div>
                <span style={{
                  fontSize: '0.65rem', fontWeight: 700,
                  padding: '3px 10px', borderRadius: 'var(--radius-full)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {activity.map((a, i) => (
                <div key={a.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <span style={{ fontWeight: 700, fontSize: '0.875rem' }}>{a.actor?.full_name} </span>
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
                      {a.action.replace('.', ' ').replace('_', ' ')}
                    </span>
                    {a.meta?.title && (
                      <span style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}> "{a.meta.title}"</span>
                    )}
                  </div>
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)' }}>
